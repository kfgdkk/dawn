 //控制层
app.controller('category_h5Controller' ,function($scope,$controller,category_h5Service){
    ...
    $scope.reloadList=function(grade){
        //切换页码
            if(grade==1){
                $scope.findByparentId(148,$scope.paginationConf.currentPage,$scope.paginationConf.itemsPerPage);
                return;
            }
	...
	$scope.findAll=function(){
        category_h5Service.findAll().success(
			function(response){
				$scope.list=response;
			}
		);
	...
	$scope.findOne=function(id){
        category_h5Service.findOne(id).success(
			function(response){
    ...
    $scope.findOne3=function(id){
        category_h5Service.findOne3(id).success(
            function(response){
                $scope.entity= response;
            }
        );
    ...
    $scope.findOne2=function(id){
        category_h5Service.findOne2(id).success(
            function(response){
                $scope.entity= response;
            }
        );
    ...
    $scope.add=function(){
        ...
        //获取选中的复选框
        category_h5Service.add( $scope.entity ).success(
            function(response){
                if(response.success){
                    //重新查询
                    alert("添加成功");
    ...
    $scope.update=function(){
        //获取选中的复选框
        category_h5Service.update( $scope.entity ).success(
            function(response){
                if(response.success){
                    //重新查询
                    alert("修改成功");
	...
	$scope.save=function(){
		...
		if($scope.entity.categordId!=null){//如果有ID
			serviceObject=category_h5Service.update( $scope.entity ); //修改
		}else{
			//在添加之前 要找到之前的父id
			$scope.entity.parentId=$scope.parentId;
			serviceObject=category_h5Service.add( $scope.entity  );//增加
		}
	...
	$scope.dele=function(){
		//获取选中的复选框
        category_h5Service.dele( $scope.selectIds ).success(
			function(response){
				if(response.success){
	...
	$scope.search=function(page,rows){
        category_h5Service.search(page,rows,$scope.searchEntity).success(
			function(response){
                $scope.paginationConf.totalItems=response.total;//更新总记录数
				$scope.list=response.rows;
	...
	$scope.findByparentId=function(parentId,page,rows){
        ...
        category_h5Service.findByparentId(parentId,page,rows).success(
		  		function(response){
...
//定义一个级别变量
    ...
    $scope.findTitleList=function(parentId,page,rows){
        ...
        $scope.parentId=parentId;//记录id
        category_h5Service.findTitleList(parentId,page,rows).success(
            function(response){
                $scope.paginationConf.totalItems = response.total;
                $scope.list=response.rows;
            }
    ...
    $scope.selectList=function(p_entity){
       ...
       // alert(p_entity.classname);
        ...
        if($scope.grade==1){
            ...
            $scope.entity_1=null;
            $scope.entity_2=null;
            $scope.findByparentId(148,$scope.page,$scope.rows);
        }
        //2级目录判断
        if($scope.grade==2){
